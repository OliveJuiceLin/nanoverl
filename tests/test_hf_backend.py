...
class HFBackendTest(unittest.TestCase):
    ...
    def test_rollout_sync_changes_log_probs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            policy_worker = HFPolicyWorker(config.model, config.actor)
            batch = RLBatch(non_tensor={"prompt_text": ["say yes"]})

            before = rollout.generate(batch, SamplingParams(do_sample=False, temperature=0.0, n=1))
            with torch.no_grad():
                first_param = next(policy_worker.model.parameters())
                first_param.add_(0.25)
            rollout.sync_policy(policy_worker.state_dict())
            after = rollout.generate(batch, SamplingParams(do_sample=False, temperature=0.0, n=1))

            self.assertNotEqual(before.batch["rollout_log_probs"], after.batch["rollout_log_probs"])
